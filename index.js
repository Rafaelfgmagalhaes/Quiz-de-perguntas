//Chamando o módulo readline-sync para o código
import input from 'readline-sync'

// coleção de dados local 
const questoes = [
...
//Pedindo ao usuário que digite seu nome através do readline-sync
let nome = input.question('Digite o seu nome: ')
console.log('\n\n');

//Embaralhando os dados local do array através da função .sort() e math.random()
const embaralhar = questoes.sort(() => Math.random() - 0.5);

let acertos = 0;
let msg = ''
// Laço de repetição para mostrar as perguntas na tela e as respectivas respostas
for(let i = 1; i <= 10; i++){

    let questao = embaralhar[i].pergunta
    console.log( i + ': ' + questao);
    let resp = input.question('Digite a resposta (ano): ');
    
    //verifica se a resposta digitada foi correta ou errada
    if(resp == embaralhar[i].resposta){
...
//condicionais para exibição da mensagem final
if(acertos <= 4){
...
console.log(`\nJogador(a): ${nome} `);
console.log(`Total de acertos: ${acertos}/10 `);
console.log(msg);
console.log('\n--------------------');